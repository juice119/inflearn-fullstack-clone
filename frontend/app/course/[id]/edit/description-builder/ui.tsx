...
export default function DescriptionBuilderUI({ course }: { course: Course }) {
  const queryClient = useQueryClient();
  const [description, setDescription] = useState<string>(course.description ?? DEFAULT_DESCRIPTION);

  const updateDescriptionMutation = useMutation({
    mutationFn: (description: string) => updateCourse(course.id, { description }),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['course', course.id] });
      toast.success('강의 상세 설명이 수정되었습니다.');
    },
    onError: (error) => {
  ...
  return (
    <div className="min-w-0">
      <div className="mb-8">
        <h1 className="text-[22px] font-bold text-foreground">상세소개</h1>
      </div>

      <div className="flex w-full flex-col gap-4">
        <div className="overflow-hidden rounded-lg border border-[#DEE2E6] bg-white [&_.ck-editor__main]:min-h-[400px] [&_.ck-content]:text-[14px] [&_.ck-content]:text-[#495057]">
          <CKEditorLazy value={description} onChange={setDescription} />
        </div>
        ...
        <div className="flex justify-end">
          <Button
            ...
            className="font-bold"
          >
            {updateDescriptionMutation.isPending ? '저장중...' : '상세소개 저장'}
          </Button>
        </div>
      </div>
    </div>
